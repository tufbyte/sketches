...
function draw()
{
	...
	robot2_y += robot2Speed_y;
        ...
        // bounce logic for robot 1
        if (robot1_x <= 0 || robot1_x + 100 >= width) {
            robot1Speed_x *= -1; // bounce horizontally
        }
        if (robot1_y <= 0 || robot1_y + 100 >= height) {
            robot1Speed_y *= -1; // bounce vertically
        }

        // bounce logic for robot 2
        if (robot2_x <= 0 || robot2_x + 100 >= width) {
            robot2Speed_x *= -1;
        }
        if (robot2_y <= 0 || robot2_y + 100 >= height) {
            robot2Speed_y *= -1;
        }
        distancex = Math.abs(robot2_x - robot1_x);
        distancey = Math.abs(robot2_y - robot1_y);
        // bounce logic between robots when robot 1 is tazed
        //if (distancey < 100){
        //    robot1Speed_y *= -1;
        //} 
        //
